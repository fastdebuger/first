          ...
          {!disabled && (
            <Popconfirm
              ...
              onConfirm={() => {
                ...
                if (onChange) onChange(filterArr);
              }}
              okText="确定"
              cancelText="取消"
            >
...
const ShowAndEditExamModal = (props: any) => {
  const { visible, onCancel, selectedSessionId } = props;
  const [form] = Form.useForm();
  const [disabled, setDisabled] = useState(true);
  const [selectedRecord, setSelectedRecord] = useState<any>(null);
  const timeTs = moment().unix(); // 当前时间时间戳
  ...
  const fetchList = async () => {
    const res = await queryExamSessionList({
      ...
      order: 'desc',
      filter: JSON.stringify([
        {Key: 'session_id', Val: selectedSessionId, Operator: '='}
      ]),
    })
    if (res.rows.length > 0) {
      ...
      let options: any[] = [];
      try {
        options = JSON.parse(record.paper_module_info || '[]')
      } catch (e) {
        options = []
      }
      setSelectedRecord(res.rows[0])
      form.setFieldsValue({
        session_name: record.session_name,
        session_description: record.session_description,
  ...
  const showDiffAlert = () => {
    ...
    // 如果当前时间 大于 开始时间，设置结束按钮，可结束考试，这样前端就看不到该考试场次了
    if (timeTs > selectedRecord.start_time) {
      return (
        <Alert message="已过考试开始时间" type="warning" showIcon
           action={
             <Space>
               <Button size="small" type="ghost" onClick={() => {
                 Modal.confirm({
                   title: '结束场次',
                   content: '结束场次后，参考人员不会看到该场次的信息',
                   okText: '确定结束',
                   cancelText: '我再想想',
                   onOk: async () => {
                     const res = await updateExamSessionStatus({
    ...
    if (selectedRecord.status === 'finished') {
      return (
        <Alert message="考试已结束" type="success" showIcon />
      )
    }
    if (selectedRecord.status === 'cancelled') {
      return (
        <Alert message="考试已取消" type="warning" showIcon />
      )
    }
    return (
      <Alert
        message="右侧按钮功能，修改：修改考试内容 ｜ 取消场次：取消本次考试 ｜ 结束：结束本场次的考试"
        type="info"
        showIcon
        action={
          <Space>
    ...
    <Modal
      ...
      footer={disabled ? null : (
        <Button type="primary" onClick={async () => {
          ...
          const res = await updateExamSession({
            session_id: selectedSessionId,
        ...
        <div>
          ...
          >
            <Form.Item name="session_name" label="考试标题" rules={[{ required: true, message: "请输入考试标题" }]}>
              <Input disabled={disabled} style={{width: '50%'}} placeholder="请输入考试标题" />
            </Form.Item>

            <Form.Item name="session_description" label="考试描述" rules={[{ required: true, message: "请输入考试描述" }]}>
              <Input.TextArea disabled={disabled} style={{width: '50%'}} placeholder="请输入考试描述" />
            </Form.Item>

            <Form.Item name="start_time" label="考试开始时间" rules={[{ required: true, message: "请选择考试开始时间" }]}>
              <FormItemIsDatePicker disabled={disabled}/>
            </Form.Item>
            ...
            >
              <FormItemIsAddModulePaper disabled={disabled}/>
            </Form.Item>
...
export default ShowAndEditExamModal;
