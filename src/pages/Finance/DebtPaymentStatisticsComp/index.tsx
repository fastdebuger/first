...
const DebtPaymentStatisticsPage: React.FC<any> = (props) => {
  ...
  const getTableColumns = () => {
    ...
    const processColumns = (columnList: any[]): any[] => {
      return columnList.map((col: any) => {
        const newCol = { ...col };
        if (newCol.title && typeof newCol.title === 'string' && newCol.title.startsWith('compinfo.')) {
          newCol.title = formatMessage({ id: newCol.title, defaultMessage: newCol.subTitle || newCol.title });
        } else if (newCol.title && typeof newCol.title === 'string') {
          // 对于动态标题（如年份），保持不变
          newCol.title = newCol.title;
        }
        // 禁用排序
        newCol.sorter = false;
        if (newCol.children) {
  ...
  const handleExport = async () => {
    try {
      ...
      wb.created = new Date();
      const ws = wb.addWorksheet(`债务填报表_${selectedKeys.length > 0 ? selectedKeys[0] : ''}`);
      ...
      const fileName = `债务填报表_${selectedKeys.length > 0 ? selectedKeys[0] : ''}_${moment().format('YYYYMMDDHHmmss')}.xlsx`;
  ...
  const renderButtonToolbar = () => {
    return (
      <Space>
        ...
        >
          {formatMessage({ id: 'compinfo.export', defaultMessage: '导出' })}
        </Button>
        <Button
          icon={<ReloadOutlined />}
          onClick={() => {
        ...
        >
          {formatMessage({ id: 'compinfo.refresh', defaultMessage: '刷新' })}
        </Button>
      </Space>
    );
  }
            ...
            <div key={selectedKeys[0]}>
              <div style={{ marginBottom: 16 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                  <strong style={{ fontSize: 18 }}>债务填报表 {selectedKeys[0]}</strong>
                  {renderButtonToolbar()}
                </div>
                {(() => {
              ...
              {dataSource.length === 0 && !loading ? (
                ...
                <div>
                  <Table
                    rowKey={(record: any) => record.id || record._rowType + '-' + Math.random()}
