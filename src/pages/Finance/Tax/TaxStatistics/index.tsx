...
import {queryFormula} from "@/services/finance/base";
  ...
  const fetchList = async () => {
    ...
    const resFormula: any = await queryFormula({
      ...
      filter: JSON.stringify([
        {Key: 'module', Val: '一般计税合计', Operator: '='},
    ...
    if(configList.length > 0) {
      ...
      res.forEach(r => {
        ...
        configList.forEach(item => {
          if (r.accounting_subject === item.accounting_subject) {
            map[item.value_added_tax_item] = r[item.total_field_name];
          } else {
            map[item.value_added_tax_item] = 0;
          }
        })
        result.push({
      ...
      const _report = {
        meta: { title: "进销平衡台账（一般计税合计）", unit: "元" },
        schema,
        rows,
      }
