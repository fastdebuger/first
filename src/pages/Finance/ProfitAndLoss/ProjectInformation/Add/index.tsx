...
import SelectDateFromEngineering
  from "@/pages/Finance/ProfitAndLoss/ProjectInformation/Common/SelectDateFromEngineering";
 ...
 * @constructor
 */
const ProjectInformationAdd: React.FC<any> = (props) => {
  const { dispatch, visible, onCancel, sysBasicDictList, callbackSuccess } = props;
      ...
      .initFormColumns([
        ...
        // "actual_start_date",
        {
          ...
          renderSelfForm: (form) => {
            const onChange = (_val) => {
              form.setFieldsValue({
                actual_start_date: _val
              })
            }
            return (
              <SelectDateFromEngineering
                title={'实际开工时间'}
                form={form}
                onChange={onChange}
                dataIndex={'actual_start_date'}
    ...
    <CrudAddModal
      ...
      onCommit={(values: any) => {
        return new Promise((resolve) => {
          ...
          dispatch({
            type: "projectInformation/addProjectInformation",
            payload: values,
            callback: (res: any) => {
              resolve(true);
