...
import DebtStatisticsSummary from "./Summary";
...
const DebtStatisticsPage: React.FC<any> = (props) => {
  ...
  const handleExport = async () => {
    try {
      ...
      // 先获取所有数据（不分页）
      const selectedQuarter = selectedKeys.length > 0 ? selectedKeys[0] : '';
      // 与表格查询保持一致：使用 like 操作符，并包含 dep_code 过滤条件
      const filter = [
      ...
      wb.created = new Date();
      const ws = wb.addWorksheet(`债权填报表_${selectedQuarter}`);
      ...
      const fileName = `债权填报表_${selectedQuarter}_${moment().format('YYYYMMDDHHmmss')}.xlsx`;
      ...
      <Button
        ...
        onClick={() => {
          ...
          Modal.confirm({
            ...
            onOk() {
              dispatch({
                type: "debtStatistics/delDebtStatistics",
                payload: {
          ...
          {selectedKeys.length > 0 && (
            <div key={selectedKeys[0]}>
              <BaseCurdSingleTable
                cRef={actionRef}
                rowKey="id"
                tableTitle={`债权填报表 ${selectedKeys[0]}`}
                type="debtStatistics/queryDebtStatistics"
                importType="debtStatistics/importDebtStatistics"
                tableColumns={getTableColumns()}
                funcCode={authority + '债权填报'}
                ...
                summary={(record: any) => <DebtStatisticsSummary dataSource={record} columns={getTableColumns()} />}
                renderSelfToolbar={() => {
