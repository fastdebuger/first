...
const WorkLicenseRegisterEdit: React.FC<any> = (props) => {
  ...
  useEffect(() => {
    ...
    fecthCityList(selectedRecord?.province_id);
  },[]);
  ...
  return (
    <CrudEditModal
      title={formatMessage({ id: 'base.user.list.edit' }) + formatMessage({ id: 'levelConstructor' })}
      visible={visible}
      onCancel={onCancel}
      initialValue={{
        ...selectedRecord,
      ...
      onCommit={async (values: any) => {
        ...
        return new Promise((resolve) => {
          dispatch({
            type: "workLicenseRegister/updateAConstructionDivision",
            payload,
            callback: (res: any) => {
              resolve(true);
              if (res.errCode === ErrorCode.ErrOk) {
