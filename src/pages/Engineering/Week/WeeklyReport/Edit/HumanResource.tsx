 ...
 * @constructor
 */
const HumanResource = ({cRef, lastWeekRecord}: any) => {
  ...
  useEffect(() => {
    // 初始化上周的数据
    if (lastWeekRecord && lastWeekRecord.WeeklyHumanResource) {
      dataSource.forEach(item => {
        const filterArr: any[] = lastWeekRecord.WeeklyHumanResource.filter(r => Number(r.person_type) === Number(item.person_type));
        filterArr.forEach(r => {
