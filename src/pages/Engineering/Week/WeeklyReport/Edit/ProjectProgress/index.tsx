...
const ProjectProgress = (props: any) => {
  ...
  useEffect(() => {
    if (lastWeekRecord && lastWeekRecord.WeeklyProProgress) {
      ...
      // 初始化 工作情况的Tab页部分
      if (lastWeekRecord.WeeklyProProgress.weeklyCompletionStatusList.length> 0) {
        form.setFieldsValue({
          weeklyCompletionStatusList: lastWeekRecord.WeeklyProProgress.weeklyCompletionStatusList
        })
      }
      // 初始化 工作情况是施工的 里的表格
      form.setFieldsValue({
        weeklyMainImageProgressList: lastWeekRecord.WeeklyProProgress.weeklyMainImageProgressList,
      })

      if (lastWeekRecord.WeeklyProProgress.weeklyEngineeringPhaseList && lastWeekRecord.WeeklyProProgress.weeklyEngineeringPhaseList.length > 0) {
        const _arr: string[] = lastWeekRecord.WeeklyProProgress.weeklyEngineeringPhaseList.map(item => item.phase);
        const _dataSource = _arr.reverse().map(type => new ProjectProgressItem(type));
        // dataSource
        _dataSource.forEach(item => {
          const findObj = lastWeekRecord.WeeklyProProgress.weeklyEngineeringPhaseList.find(r => r.phase === item.phase)
