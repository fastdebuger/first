 ...
 * @constructor
 */
const MasterEquipmentResource = ({ cRef, lastWeekRecord }: any) => {
  ...
  useEffect(() => {
    if (lastWeekRecord && lastWeekRecord.WeeklyEquipments) {
      console.log("lastWeekRecord.WeeklyEquipments", lastWeekRecord.WeeklyEquipments);
      const filterArr1 = lastWeekRecord.WeeklyEquipments.filter(r => r.equipment_type === '通用设备')
      ...
      const filterArr2 = lastWeekRecord.WeeklyEquipments.filter(r => r.equipment_type === '管道专用设备')
      ...
      if (col.valueType === 'input') {
        Object.assign(col, {
          render: (text: string, record: any) => {
            return (
              <Text>{record.getField(col.dataIndex)}</Text>
            )
          }
        })
      }
