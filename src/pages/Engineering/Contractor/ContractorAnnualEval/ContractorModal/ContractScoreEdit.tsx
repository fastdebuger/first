...
import { Spin, message, InputNumber, Drawer, Row, Form, Col, Button, Tag, Card } from "antd";
 ...
 * @constructor
 */
const ContractScoreEdit: React.FC<any> = (props) => {
  ...
  */
  const onCommit = async () => {
    ...
    return new Promise((resolve) => {
      dispatch({
        type: "appraiseInfo/updateOutContractScoreInfo",
        payload,
        callback: (res: any) => {
          resolve(true);
          if (res.errCode === ErrorCode.ErrOk) {
            // 提示消息,使用message，并增加zIndex层级时期能成功显示
            message.success({
              content: `编辑评分成功！`,
              duration: 2,
              style: {
...
export default connect()(ContractScoreEdit);
