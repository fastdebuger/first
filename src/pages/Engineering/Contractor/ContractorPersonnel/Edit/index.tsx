 ...
 * @constructor
 */
const PersonInfoEdit: React.FC<any> = (props) => {
  ...
  const getFormColumns = () => {
    let formColumns:any = [];
    if([0,1].includes(Number(selectedRecord?.status))) {
      formColumns.push('status')
    }
    ...
    <CrudEditModal
      ...
      onCommit={(values: any) => {
        return new Promise((resolve) => {
          dispatch({
            type: "personInfo/updatePersonInfo",
            payload: {
              ...selectedRecord,
              ...values,
            },
...
export default connect()(PersonInfoEdit);
