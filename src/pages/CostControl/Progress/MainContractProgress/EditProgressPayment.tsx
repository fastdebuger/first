...
/**
 * 修改进度款组件（动态生成字段：is_arrivalX、approval_amountX、file_urlX）
 * 根据 approval_amountX / is_arrivalX 是否存在计算需要渲染的序号
 */
const EditProgressPayment: React.FC<EditProgressPaymentProps> = (props) => {
  ...
  /**
   * 计算需要渲染的序号集合
   * 采集存在 approval_amount{X} 或 is_arrival{X} 且 is_arrival{X} 不为 1 的索引 X（未到账的进度款可编辑）
  ...
  const getIndices = (): number[] => {
    ...
    Object.keys(selectedRecord).forEach((key) => {
      const match = key.match(/^approval_amount(\d+)$/) || key.match(/^is_arrival(\d+)$/);
      if (match) {
        const idx = Number(match[1]);
        const isArrivalValue = selectedRecord[`is_arrival${idx}`];
        if (isArrivalValue !== '1' && isArrivalValue !== '') {
          result.push(idx);
        }
      }
    });
  ...
  /**
   * 生成初始回显值
   * 根据传入的序号集合，回显 is_arrivalX、approval_amountX、file_urlX
        ...
        <CrudEditModal
          ...
          onCommit={(values: any) => {
            return new Promise((resolve) => {
              dispatch({
                type: 'costControl/updateProgressPaymentBody',
                payload: {
                  ...values,
