...
export const getFieldDisabled = (category: string | undefined, fieldName: string) => {
  ...
  switch (category) {
    ...
    case '2': // 作业行为类
      // 启用：operation_behavior_ids, question_category
      // 禁用：其他所有
      return !['operation_behavior_ids', 'question_category'].includes(fieldName);

    case '3': // 安全管理类
      // 启用：safety_factor1, safety_factor2
      // 禁用：其他所有
      return !['safety_factor1', 'safety_factor2'].includes(fieldName);
...
export const getRequiredFields = (category: string | undefined) => {
  ...
  switch (category) {
    ...
    case '2': // 作业行为类
      return [...baseRequiredFields, 'operation_behavior_ids', 'question_category'];

    case '3': // 安全管理类
      return [...baseRequiredFields, 'safety_factor1', 'safety_factor2'];
