...
import { buildApiParams, generateChartData, processTableData, TableDataItem } from './problemClassificationStatisticsUtils';
import { useInitData } from './hooks';
import { downloadChart, getDefaultTimePeriod } from '../problemStatisticsUtils';
import { getProblemClassificationStatistics } from '@/services/safetyGreen/inspect/problemStatistics';
...
/**
 * 问题归类统计
 * 统计问题按问题类型和问题归类的情况，以柱状图展示
 */
const ProblemClassificationStatistics: React.FC<any> = (props) => {
  ...
  const getColumns = (): any[] => {
    const baseColumns: any[] = [
      ...
      { title: '问题类型 (质量、HSE)', dataIndex: 'problemType', key: 'problemType', width: 150 },
    ];

    // 判断是否选择了问题归类：选择了则添加二级检索词列
    const hasProblemCategory = Boolean(searchParams.problemCategory);
    hasProblemCategory && baseColumns.push({
      title: '二级检索词 (质量管理类、质量实体类、安全管理类、作业行为类)',
      dataIndex: 'secondaryTerm',
      key: 'secondaryTerm',
      width: 300,
    });
  ...
  const handleDataResponse = (res: any, currentParams: SearchParams) => {
    ...
    // 成功时：生成图表数据和表格数据
    setChartData(generateChartData(res.result, currentParams.problemCategory, currentParams.problemType));
    const processedTableData = processTableData(res.result, currentParams);
  ...
  const fetchData = async (customParams?: SearchParams) => {
    ...
    const res = await getProblemClassificationStatistics(params);
    ...
    <div>
          ...
          style={{ marginTop: 16 }}
            ...
            >
              <Spin spinning={loading} tip="加载中...">
                <ReactECharts
                  ref={chartRef}
              key={`classification-chart-${chartKey}`}
      ...
      >
        <Table
          ...
          summary={(pageData) => {
            const totalQuantity = pageData.reduce((sum, record) => sum + (record.problemQuantity || 0), 0);
            return (
              <Table.Summary fixed>
                <Table.Summary.Row style={{ backgroundColor: '#fafafa' }}>
                  <Table.Summary.Cell index={0} colSpan={searchParams.problemCategory ? 4 : 3} align="center">
                    <strong>合计</strong>
                  </Table.Summary.Cell>
...
export default connect()(ProblemClassificationStatistics);
