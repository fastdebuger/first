...
/**
 * 合同号选择弹窗组件（带搜索、表格、单选功能）
 * 用途：用于在表单中用项目中快速选择一个合同号（合同编号）
 * @param props.value           当前已选中的合同对象（父组件控制）
 * @param props.handleChange    选择或清除后的回调函数，参数为选中记录或 null
 * @param props.addedDataSource 父组件中已经添加的合同列表（用于去重判断）
 */
const ContractNoModal: React.FC<any> = (props: any) => {
  const { value, handleChange, dataSource: addedDataSource, disabled } = props;
        ...
        isVisible && (
          ...
          >
            <BaseCurdSingleTable
              ...
              selectedRowsToolbar={() => []}
              // queryAlternativeInfo 该字段后台可以根据该参数过滤出已经选过的数据，将其过滤出去默认传递1
              tableDefaultField={{ queryAlternativeInfo: 1 }}
              tableDefaultFilter={
...
export default connect()(ContractNoModal);
