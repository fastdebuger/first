...
export const useInitData = (dispatch: any) => {
  ...
  useEffect(() => {
    if (dispatch) {
      ...
      dispatch({
        ...
        callback: (res: any) => {
          if (res && res.rows) {
            setProblemClassificationData(res.rows || []);
          }
        }
      });
    }
  ...
  return { obsCodeAllData, wbsOptions, branchCompOptions, problemClassificationData };
};

/**
 * 根据分公司动态加载项目部的hook
 */
export const useProjectWbsOptions = (branchCompCode: string | undefined, dispatch: any) => {
  const [projectWbsOptions, setProjectWbsOptions] = useState<any[]>([]);

  useEffect(() => {
    if (branchCompCode && dispatch) {
      dispatch({
        type: 'user/queryWBS',
        payload: {
          sort: 'wbs_code',
