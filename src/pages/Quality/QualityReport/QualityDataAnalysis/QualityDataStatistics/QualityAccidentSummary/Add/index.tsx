...
// 引入当前模块的列配置（定义了表单字段的元信息）
import { configColumns } from "../columns";
// 从 yayang-ui 引入表单列构建工具和基础 CRUD 模态框组件
import { BasicFormColumns, SingleTable } from "yayang-ui";
// 引入 Umi 的国际化钩子 useIntl 和 connect（用于 Redux）
import { useIntl, connect } from "umi";
// 引入全局错误码常量，用于判断接口响应状态
import { ErrorCode } from "@/common/const";
// 引入 Ant Design 的消息提示组件和 Form
import { message, Form } from "antd";
// 引入华为云 OBS 存储的系统路径配置常量
import { HUA_WEI_OBS_CONFIG } from "@yayang/constants";
// 引入自定义的华为云文件上传组件
import HuaWeiOBSUploadFile from "@/components/HuaWeiOBSUploadFile";

// 从 SingleTable 中解构出新增模态框组件
 ...
 *   - visible: 控制弹窗是否显示
 *   - onCancel: 关闭弹窗回调
 *   - callbackSuccess: 新增成功后的回调（用于刷新父表格）
    ...
    <CrudAddModal
      ...
      onCommit={(values: any) => {
        ...
        return new Promise((resolve) => {
          // 发起新增请求
          dispatch({
            type: "qualityAccidentSummary/addQualityAccidentSummary",
            payload: values,             // 表单收集到的所有字段值
            callback: (res: any) => {
              resolve(true);             // 告诉 CrudAddModal 请求已完成（无论成功失败）
