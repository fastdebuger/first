...
/**
 * 起重机械安装修理质量保证体系责任人员推荐表编辑
 * @param props 
 * @returns 
 */
const HMQAStaffNominationEdit: React.FC<any> = (props: any) => {
  ...
  useEffect(() => {
    //获取表格数据
    dispatch({
      type: "hmQAStaffNomination/getBody",
      payload: {
        sort: 'id',
        order: 'asc',
  ...
  useEffect(() => {
    dispatch({
      ...
      payload: {
        ...
        order: 'asc',
        filter: JSON.stringify([
          { Key: 'special_equip_type', Val: "4", Operator: '=' },
        ])
      },
      callback: (res: any) => {
      ...
      .setFormColumnToSelect([
        {
          ...
          data: [
            {
              construction_category: "安装",
              construction_category_str: "安装",
            },
            {
              construction_category: "修理",
              construction_category_str: "修理",
            }
          ],
        }
      ])
    ...
    <CrudEditModal
      ...
      onCommit={(data: any) => {
        ...
        return new Promise((resolve) => {
          ...
          const result = SpecialEquipmentVerifyService.validate("5", dataSource);
          if (!result.success) {
            message.warning({
              content: result.msg,
              duration: 5,
          ...
          dispatch({
            type: "hmQAStaffNomination/updateInfo",
            payload: {
              ...selectedRecord,
              ...values,
...
export default connect()(HMQAStaffNominationEdit);