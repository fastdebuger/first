...
import PCQAStaffNominationAdd from "./Add";
import PCQAStaffNominationDetail from "./Detail";
import PCQAStaffNominationEdit from "./Edit";
...
/**
 * 压力管道元件制造质量保证体系责任人员推荐表
 * @constructor
 */
const PCQAStaffNominationnPage: React.FC<any> = (props) => {
  ...
  // 根据项目部填报判断是否可以新增
  const isPCQAStaffNominationn = useRef(false)
  ...
  useEffect(() => {
    dispatch({
      ...
      callback: (res: any) => {
        isPCQAStaffNominationn.current = res?.rows?.length > 0;
      },
    });
  }, [])
    ...
    return [
      <Space>
        <Button
          ...
          type="primary"
          onClick={() => {
            if (isPCQAStaffNominationn.current) {
              setAddVisible(true);
            } else {
  ...
  const renderSelectedRowsToolbar = (selectedRows: any[], reloadTable: (filters?: [], noFilters?: []) => void) => {
    ...
    return [
      ...
      hasPermission(authority, '发起审批') && <InitiateApproval
        ...
        dispatch={dispatch}
        funcode={APPROVAL_FUNCODE.PipelineComponentQAStaffRecommendationApproval}
        type='pcQAStaffNomination/sendApproval'
        onSuccess={() => {
          if (actionRef.current) {
      ...
      <ViewApproval
        ...
        funcCode={APPROVAL_FUNCODE.PipelineComponentQAStaffRecommendationApproval}
        selectedRecord={selectedRecord}
        onSuccess={() => {
          if (actionRef.current) {
            actionRef.current.reloadTable();
      ...
      />,
      <ViewTheReport
        query={`?viewlet=WeldSys2.0/Quality/Component-Warranty.cpt&special_equip_type=${selectedRecord?.special_equip_type}&main_id=${selectedRecord?.main_id}`}
      />,
      <Button
        danger
  ...
  return (
    <div>
      <BaseCurdSingleTable
        cRef={actionRef}
        rowKey="main_id"
        tableTitle='压力管道元件制造质量保证体系责任人员推荐表'
        moduleCaption="压力管道元件制造质量保证体系责任人员推荐表"
        type="pcQAStaffNomination/getHead"
        exportType="pcQAStaffNomination/getHead"
        tableColumns={getTableColumns()}
        funcCode={authority + "压力管道元件制造质量保证体系责任人员推荐表"}
...
export default connect()(PCQAStaffNominationnPage);
