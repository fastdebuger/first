...
import QualitySafetyWeekCheckAdd from "./Add";
import QualitySafetyWeekCheckDetail from "./Detail";
 ...
 * @constructor
 */
const QualitySafetyWeekCheckPage: React.FC<any> = (props) => {
  const { dispatch, route: { authority }, special_equip_type, title } = props;
  ...
  // 每周任务是否可以填报 根据每天的填报进行判断
  const isComplete = useRef(0);
  useEffect(() => {
    dispatch({
      type: "qualitySafetyWeekCheck/checkDailyReport",
      payload: {
        sort: "id",
        "report_date": moment().format("YYYY-MM-DD"),
      },
      callback: (res: any) => {
        const result = res.is_complete
        isComplete.current = (String(result) === "1" ? 1 : 0)
      }
    })
  }, []);
        ...
        <Button
          ...
          onClick={() => {
            if (isComplete.current === 0) {
              message.warning("请完成本周每日报送后再进行提交");
              return
            }
            setAddVisible(true);
          }}
  ...
  const renderSelectedRowsToolbar = (selectedRows: any[], reloadTable: (filters?: [], noFilters?: []) => void) => {
    ...
    return [
      ...
      hasPermission(authority, '发起审批') && <InitiateApproval
        ...
        dispatch={dispatch}
        funcode={APPROVAL_FUNCODE.PressureVesselWeeklyCheckApproval}
        type='qualitySafetyWeekCheck/sendApproval'
        onSuccess={() => {
          if (actionRef.current) {
      ...
      <ViewApproval
        ...
        funcCode={APPROVAL_FUNCODE.PressureVesselWeeklyCheckApproval}
        selectedRecord={selectedRecord}
        onSuccess={() => {
          if (actionRef.current) {
            actionRef.current.reloadTable();
      ...
      />,
      <ViewTheReport
        query={`?viewlet=WeldSys2.0/Quality/Quality-Governance.cpt&special_equip_type=${selectedRecord?.special_equip_type}&id=${selectedRecord?.id}`}
      />,
      <Button
        danger
  ...
  return (
    <div>
      <BaseCurdSingleTable
        ...
        rowKey="id"
        tableTitle={title}
        type="qualitySafetyWeekCheck/getInfo"
        exportType="qualitySafetyWeekCheck/getInfo"
...
export default connect()(QualitySafetyWeekCheckPage);
