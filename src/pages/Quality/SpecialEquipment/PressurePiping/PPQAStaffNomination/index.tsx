...
import PPQAStaffNominationAdd from "./Add";
import PPQAStaffNominationDetail from "./Detail";
import PPQAStaffNominationEdit from "./Edit";
 ...
 * @constructor
 */
const PPQAStaffNominationPage: React.FC<any> = (props) => {
  ...
  // 根据项目部填报判断是否可以新增
  const isPPQAStaffNomination = useRef(false)
  ...
  useEffect(() => {
    dispatch({
      ...
      callback: (res: any) => {
        isPPQAStaffNomination.current = res?.rows?.length > 0;
      },
    });
  }, [])
    ...
    return [
      <Space>
        <Button
          ...
          type="primary"
          onClick={() => {
            if (isPPQAStaffNomination.current) {
              setAddVisible(true);
            } else {
  ...
  const renderSelectedRowsToolbar = (selectedRows: any[], reloadTable: (filters?: [], noFilters?: []) => void) => {
    ...
    return [
      ...
      hasPermission(authority, '发起审批') && <InitiateApproval
        ...
        dispatch={dispatch}
        funcode={APPROVAL_FUNCODE.PressurePipelineQAStaffRecommendationApproval}
        type='ppQAStaffNomination/sendApproval'
        onSuccess={() => {
          if (actionRef.current) {
      ...
      <ViewApproval
        ...
        funcCode={APPROVAL_FUNCODE.PressurePipelineQAStaffRecommendationApproval}
        selectedRecord={selectedRecord}
        onSuccess={() => {
          if (actionRef.current) {
            actionRef.current.reloadTable();
      ...
      />,
      <ViewTheReport
        query={`?viewlet=WeldSys2.0/Quality/Pipeline-Warranty.cpt&special_equip_type=${selectedRecord?.special_equip_type}&main_id=${selectedRecord?.main_id}`}
      />,
      <Button
        danger
        ...
        onClick={() => {
          ...
          Modal.confirm({
            ...
            onOk() {
              dispatch({
                type: "ppQAStaffNomination/delInfo",
                payload: {
  ...
  return (
    <div>
      <BaseCurdSingleTable
        cRef={actionRef}
        rowKey="main_id"
        tableTitle='压力管道安装质量保证体系责任人员推荐表'
        moduleCaption="压力管道安装质量保证体系责任人员推荐表"
        type="ppQAStaffNomination/getHead"
        exportType="ppQAStaffNomination/getHead"
        tableColumns={getTableColumns()}
        funcCode={authority + "压力管道安装质量保证体系责任人员推荐表"}
...
export default connect()(PPQAStaffNominationPage);
