...
/**
 * 压力容器制造(组焊、安装改造修理)质保体系责任人员推荐表
 * @constructor
 */
const PVQAStaffNominationPage: React.FC<any> = (props) => {
  ...
  // 根据项目部填报判断是否可以新增
  const isPVQAStaffNomination = useRef(false)
  ...
  useEffect(() => {
    dispatch({
      ...
      callback: (res: any) => {
        isPVQAStaffNomination.current = res?.rows?.length > 0;
      },
    });
  }, [])
    ...
    return [
      <Space>
        <Button
          ...
          type="primary"
          onClick={() => {
            if (isPVQAStaffNomination.current) {
              setAddVisible(true);
            } else {
  ...
  const renderSelectedRowsToolbar = (selectedRows: any[], reloadTable: (filters?: [], noFilters?: []) => void) => {
    ...
    return [
      ...
      hasPermission(authority, '发起审批') && <InitiateApproval
        ...
        dispatch={dispatch}
        funcode={APPROVAL_FUNCODE.PressureVesselQAStaffRecommendationApproval}
        type='PVQAStaffNomination/sendApproval'
        onSuccess={() => {
          if (actionRef.current) {
      ...
      <ViewApproval
        ...
        funcCode={APPROVAL_FUNCODE.PressureVesselQAStaffRecommendationApproval}
        selectedRecord={selectedRecord}
        onSuccess={() => {
          if (actionRef.current) {
            actionRef.current.reloadTable();
      ...
      />,
      <ViewTheReport
        query={`?viewlet=WeldSys2.0/Quality/Vessel-Warranty.cpt&special_equip_type=${selectedRecord?.special_equip_type}&main_id=${selectedRecord?.main_id}`}
      />,
      <Button
        danger
        ...
        onClick={() => {
          ...
          Modal.confirm({
            ...
            onOk() {
              dispatch({
                type: "PVQAStaffNomination/delInfo",
                payload: {
  ...
  return (
    <div>
      <BaseCurdSingleTable
        cRef={actionRef}
        rowKey="main_id"
        tableTitle='压力容器制造(组焊、安装改造修理)质保体系责任人员推荐表'
        moduleCaption="压力容器制造(组焊、安装改造修理)质保体系责任人员推荐表"
        type="PVQAStaffNomination/getHead"
        exportType="PVQAStaffNomination/getHead"
        tableColumns={getTableColumns()}
        funcCode={authority + "压力容器制造(组焊、安装改造修理)质保体系责任人员推荐表"}
...
export default connect()(PVQAStaffNominationPage);
