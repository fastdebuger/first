...
import PressureVesselPreformanceAdd from "./Add";
import PressureVesselPreformanceDetail from "./Detail";
...
/**
 * 压力容器制造(组焊、安装改造修理)施工业绩表
 * @constructor
 */
const PressureVesselPreformancePage: React.FC<any> = (props) => {
  ...
  const renderSelectedRowsToolbar = (selectedRows: any[], reloadTable: (filters?: [], noFilters?: []) => void) => {
    ...
    return [
      ...
      hasPermission(authority, '发起审批') && <InitiateApproval
        ...
        dispatch={dispatch}
        funcode={APPROVAL_FUNCODE.PressureVesselConstructionPerformanceApproval}
        type='pressureVesselPreformance/sendApproval'
        onSuccess={() => {
          if (actionRef.current) {
      ...
      <ViewApproval
        ...
        funcCode={APPROVAL_FUNCODE.PressureVesselConstructionPerformanceApproval}
        selectedRecord={selectedRecord}
        onSuccess={() => {
          if (actionRef.current) {
            actionRef.current.reloadTable();
  ...
  return (
    <div>
      <BaseCurdSingleTable
        cRef={actionRef}
        rowKey="id"
        tableTitle='压力容器制造(组焊、安装改造修理)施工业绩表'
        moduleCaption='压力容器制造(组焊、安装改造修理)施工业绩表'
        type="pressureVesselPreformance/getInfo"
        exportType="pressureVesselPreformance/getInfo"
        tableColumns={getTableColumns()}
        funcCode={authority + "压力容器制造(组焊、安装改造修理)施工业绩表"}
      ...
      />
      {open && selectedRecord && (
        <PressureVesselPreformanceDetail
          open={open}
          actionRef={actionRef}
          selectedRecord={selectedRecord}
          authority={authority}
...
export default connect()(PressureVesselPreformancePage);
