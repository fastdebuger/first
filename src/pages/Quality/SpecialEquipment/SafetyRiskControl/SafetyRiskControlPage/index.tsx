 ...
 * @constructor
 */
const SafetyRiskControlPage: React.FC<any> = (props) => {
  const { dispatch, route: { authority }, title = '特种设备质量安全风险管控清单', special_equip_type = "1" } = props;
  ...
  const renderSelectedRowsToolbar = (selectedRows: any[], reloadTable: (filters?: [], noFilters?: []) => void) => {
    ...
    return [
      ...
      hasPermission(authority, '发起审批') && <InitiateApproval
        ...
        dispatch={dispatch}
        funcode={APPROVAL_FUNCODE.PressureVesselRiskControlListApproval}
        type='SafetyRiskControl/sendApproval'
        onSuccess={() => {
          if (actionRef.current) {
      ...
      <ViewApproval
        ...
        funcCode={APPROVAL_FUNCODE.PressureVesselRiskControlListApproval}
        selectedRecord={selectedRecord}
        onSuccess={() => {
          if (actionRef.current) {
            actionRef.current.reloadTable();
      ...
      />,
      <ViewTheReport
        query={`?viewlet=WeldSys2.0/Quality/Quality-Control.cpt&special_equip_type=${selectedRecord?.special_equip_type}&main_id=${selectedRecord?.main_id}`}
      />,
      <Button
        danger
  ...
  return (
    <div>
      <BaseCurdSingleTable
        ...
        tableTitle={title}
        moduleCaption={title}
        type="SafetyRiskControl/getHead"
        exportType="SafetyRiskControl/getHead"
...
export default connect()(SafetyRiskControlPage);
