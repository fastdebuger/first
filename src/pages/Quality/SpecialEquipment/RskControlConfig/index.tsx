...
const PostStructurePage: React.FC<any> = (props) => {
  ...
  const onSelect = (selectedKeys: any) => {
    ...
    if (node) {
      setSelectedNode({
        id: node.id,
        dict_name: node.dict_name,
        dict_type: node.dict_type,
        dict_type_name: node.dict_type_name,
  ...
  const loop = (data: any[]): any[] =>
    data.map((item) => {
      const index = item.dict_name.indexOf(searchValue);
      const beforeStr = item.dict_name.substring(0, index);
      const afterStr = item.dict_name.substring(index + searchValue.length);
      const title =
        index > -1 ? (
          <span>
            {beforeStr}
  ...
  const onFinish = (values: any) => {
    ...
    setBtnLoading(true);
    dispatch({
      type: 'RiskControlConfig/updateInfo',
      payload: {
        ...selectedNode,
        ...values,
      },
  ...
  const handleDel = (id: string) => {
    setBtnLoading(true);
    dispatch({
      type: 'RiskControlConfig/delInfo',
      payload: { id },
      callback: (res: any) => {
        setBtnLoading(false);
  ...
  return (
    <div style={{ backgroundColor: '#fff', display: 'flex', padding: '20px', minHeight: '80vh' }}>
      ...
      <div style={{ width: '30%', borderRight: '1px solid #f0f0f0', paddingRight: '20px' }}>
        <Spin spinning={pageLoading}>
          <Divider orientation="left" plain>
            <h3 style={{ margin: 0 }}>特种设备质量安全风险管控清单配置表</h3>
          </Divider>
      ...
      <div style={{ width: '70%', padding: '40px' }}>
        ...
        {selectedNode.id ? (
          <Card title={`编辑: ${selectedNode.dict_name}`}>
