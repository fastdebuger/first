...
/**
 * 锅炉安装改造修理质量保证体系责任人员推荐表
 * @constructor
 */
const BoilerQAStaffNominationPage: React.FC<any> = (props) => {
  ...
  // 根据项目部填报判断是否可以新增
  const isBoilerQAStaffNomination = useRef(false)
  ...
  useEffect(() => {
    dispatch({
      ...
      callback: (res: any) => {
        isBoilerQAStaffNomination.current = res?.rows?.length > 0;
      },
    });
  }, [])
    ...
    return [
      <Space>
        <Button
          ...
          type="primary"
          onClick={() => {
            if (isBoilerQAStaffNomination.current) {
              setAddVisible(true);
            } else {
  ...
  const renderSelectedRowsToolbar = (selectedRows: any[], reloadTable: (filters?: [], noFilters?: []) => void) => {
    ...
    return [
      ...
      hasPermission(authority, '发起审批') && <InitiateApproval
        ...
        dispatch={dispatch}
        funcode={APPROVAL_FUNCODE.BoilerQAStaffRecommendationApproval}
        type='boilerQAStaffNomination/sendApproval'
        onSuccess={() => {
          if (actionRef.current) {
      ...
      <ViewApproval
        ...
        funcCode={APPROVAL_FUNCODE.BoilerQAStaffRecommendationApproval}
        selectedRecord={selectedRecord}
        onSuccess={() => {
          if (actionRef.current) {
            actionRef.current.reloadTable();
      ...
      />,
      <ViewTheReport
        query={`?viewlet=WeldSys2.0/Quality/Boiler-Warranty.cpt&special_equip_type=${selectedRecord?.special_equip_type}&main_id=${selectedRecord?.main_id}`}
      />,
      <Button
        danger
        ...
        onClick={() => {
          ...
          Modal.confirm({
            ...
            onOk() {
              dispatch({
                type: "boilerQAStaffNomination/delInfo",
                payload: {
  ...
  return (
    <div>
      <BaseCurdSingleTable
        cRef={actionRef}
        rowKey="main_id"
        tableTitle='锅炉安装改造修理质量保证体系责任人员推荐表'
        moduleCaption="锅炉安装改造修理质量保证体系责任人员推荐表"
        type="boilerQAStaffNomination/getHead"
        exportType="boilerQAStaffNomination/getHead"
        tableColumns={getTableColumns()}
        funcCode={authority + "锅炉安装改造修理质量保证体系责任人员推荐表"}
...
export default connect()(BoilerQAStaffNominationPage);
