...
import { materialsColumns, serviceColumns, subcontractColumns } from "../columns";
import PurchaseStrategySelect from "../Add/PurchaseStrategySelect";
 ...
 * @returns 
 */
const PurchaseStrategyLotPlanEdit: React.FC<any> = (props: any) => {
  const { dispatch, visible, onCancel, callbackEditSuccess, selectedRecord, activeTab } = props;
  ...
  const pageTitle = `编辑${titlePrefix}单个标段策划方案`;
  ...
  useEffect(() => {
    ...
    dispatch({
      ...
      payload: {
        sort: "id",
        order: "desc",
        filter: JSON.stringify([{ Key: "form_no", Val: selectedRecord.form_no, Operator: "=" }]),
      },
      callback: (res: any) => {
  ...
  }, [dispatch, selectedRecord?.form_no]);
  ...
  const getTableColumns = () => {
    const cols = new BasicEditableColumns(baseColumns)
      ...
      .setTableColumnToInputNumber([{ value: "proposed_qty", valueType: "digit" }])
      .needToRules([
        "supplier_name",
        "supplier_source",
        "supplier_reg_addr",
  ...
  return (
    <CrudEditModal
      ...
      initFormValues={{ ...selectedRecord, materials_type: String(selectedRecord.materials_type) }}
      formColumns={getFormColumns()}
      tableColumns={getTableColumns()}
      initDataSource={initTableData}
      onCommit={(data: any) => {
        const { addItems, editItems, delItems, form } = data;
        const values = form.getFieldsValue();
        return new Promise((resolve) => {
          dispatch({
            type: "purchaseStrategyLotPlan/updatePurchaseStrategyLotPlan",
            payload: {
              ...selectedRecord,
              ...
              DelItems: JSON.stringify(
                delItems.reduce((result: any[], item: any) => {
                  result.push(item.form_no ?? item.id);
                  return result;
                }, []),
              ),
            },
...
export default connect()(PurchaseStrategyLotPlanEdit);